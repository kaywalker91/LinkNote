...
- **Session 64 (RLS 문서 PR)**: `scripts/migration_63_visibility_rls.sql` 신설 + `docs/security/rls-policies.md`에 "visibility 컬럼과 RLS" 섹션 추가. 결정: 컬렉션은 **strictly owner-only** 유지 → `private`는 이미 강제됨(비소유자는 visibility 무관 read 불가), `public`은 **백엔드 무효과**(pill·토글 표시용)로 의도된 설계. `public` 비소유자 read 허용은 공유/공개 뷰 기능 구현 시 별도 결정. ⚠️ 라이브 DB enable 검증은 대시보드 액세스 blocked. 이 PR에 직전 wrap 문서 동봉.
...
- **Collection 카드 Lock/Globe pill** (M) — `CollectionEntity` visibility 필드는 이미 존재(#46/#47). 카드 표면 pill만 추가. Phase 5+.
...
flutter test --reporter=failures-only         # 604 GREEN (2026-06-20 실측, 골든 포함 — #53 이후 --exclude-tags golden 불필요)
